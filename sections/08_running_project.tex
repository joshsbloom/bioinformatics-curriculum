\chapter{Running Project: From Growth Curves to QTL Mapping}

This four-part project follows a single scientific narrative: can you find the genetic basis of a yeast strain's response to an unknown compound? Each problem set is designed to be worked on during or shortly after the block that teaches the relevant skills. The problems build on each other, so work through them in order.

All data can be found in the GitHub repository \texttt{joshsbloom/QuantGenSandbox} and on the linked Dropbox. The project culminates in a QTL mapping scan that identifies a candidate gene and generates a hypothesis about the compound's identity.
...
\midrule
1. Yeast Growth Curves & Blocks 5--6 (R) & R scripting, ggplot2, t-tests, permutation tests, power simulation \\
2. Heritability \& Mixed Models & Blocks 7--8 (Stats) & Linear models, random effects, heritability, experimental design \\
3. Short-Read Alignment \& Strain ID & Blocks 9--10 (Apps) & Unix pipelines, bwa, sambamba, GATK, BLAST, Hamming distance \\
4. QTL Mapping & Blocks 11--12 (Capstone) & Genome-wide scans, permutation thresholds, LOD scores, confidence intervals \\
\bottomrule
\end{tabularx}
\end{center}
...
%% ────────────────────────────────────────────
\section{Problem Set 1: Yeast Growth Curves}
...
\begin{dobox}[Tasks]
\begin{itemize}[nosep]
  \item Use ggplot2 to make a histogram of doubling times by strain (colored by strain).
  \item Make a violin plot showing individual data points, mean, and 95\% confidence interval of the mean for each strain.
  \item Use \texttt{t.test()} to test for a difference in means. Think about whether a paired t-test is appropriate here and why or why not.
\end{itemize}
\end{dobox}
...
%% ────────────────────────────────────────────
\section{Problem Set 2: Heritability \& Mixed Models}
...
%% ────────────────────────────────────────────
\section{Problem Set 3: Short-Read Alignment \& Strain Identification}
...
%% ────────────────────────────────────────────
\section{Problem Set 4: QTL Mapping}
